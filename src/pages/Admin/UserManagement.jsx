...
import { UserPlus, Edit2, Shield, MoreVertical, Search, ShieldCheck, Users } from 'lucide-react';

const initialUserRegistry = [
  ...
  { id: "USR-004", name: "Bella Inamahoro", email: "bella.i@mukopo.com", role: "Manager", dept: "Design", status: "Invité" }
];
...
export default function UserManagement() {
  ...
  const filteredUsers = initialUserRegistry.filter(account => {
    const matchesSearch = account.name.toLowerCase().includes(search.toLowerCase()) || 
                          account.email.toLowerCase().includes(search.toLowerCase()) ||
                          account.dept.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-muko-card border border-slate-800/80 rounded-2xl p-5 flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xl font-black text-white font-mono">{initialUserRegistry.length} comptes</p>
          </div>
            ...
            <tbody className="divide-y divide-slate-800/60 text-xs text-slate-300">
              {filteredUsers.length > 0 ? (
                filteredUsers.map((account) => (
                  <tr key={account.id} className="hover:bg-[#0B131F]/30 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${
                        account.status === 'Actif' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
