...
export default function EditExpense() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      alert(err.response?.data?.message || "Impossible de supprimer la note.");
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <form onSubmit={handleUpdate} className="bg-[#111C2E] border border-slate-800/80 rounded-2xl p-8 space-y-6">
        <div className="border-b border-slate-800/60 pb-4">
          <h2 className="text-base font-bold text-white">Modifier la note de frais</h2>
          <p className="text-xs text-slate-500 mt-0.5">ID : <span className="text-[#FF6B2C] font-mono font-bold">{id?.substring(id.length - 8).toUpperCase()}</span></p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-[#1A263B] text-slate-200 text-xs rounded-xl p-3.5 border border-slate-800 outline-none focus:border-[#FF6B2C]/50 transition-colors"
              required
            >
          ...
          />
          <div>
            <label className="text-[10px] font-mono uppercase tracking-wider text-slate-400 block mb-2">Devise</label>
            <select
              value={form.currency}
